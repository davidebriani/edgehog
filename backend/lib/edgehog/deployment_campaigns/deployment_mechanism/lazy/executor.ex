...
defmodule Edgehog.DeploymentCampaigns.DeploymentMechanism.Lazy.Executor do
  ...
  def handle_event(:info, {:deployment_updated, deployment}, _state, data) do
    ...
    additional_actions =
      case Core.deployment_ready?(deployment) do
        :ready ->
          [internal_event({:deployment_success, deployment})]

        {:not_ready, _missing_resources} ->
          []
      end
  ...
  defp terminate_executor(deployment_campaign_id) do
    Logger.info("Terminating executor process for Update Campaign #{deployment_campaign_id}")
    {:stop, :normal}
  end
